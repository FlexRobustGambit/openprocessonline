	...
	if (typeof(document.designMode) == "string" && (document.all || document.designMode == "off")){
		...
		for (var i = 0; i < theTextareas.length; i++){
			...
			if (theTextarea.className.classExists("widgEditor")){
				...
				setTimeout("new widgEditor('" + theTextarea.id + "')", 100 * (i));
			}
		}
	}else{
		return false;
...
function widgEditor(replacedTextareaID)
{
	...
	this.theBody = document.getElementById("iframeBody");
	this.IE = false;
	this.locked = true;
	this.pasteCache = "";
	this.wysiwyg = true;
	...
	if (typeof (Storage) !== "undefined") {
	    this.theIframe.contentWindow.document.getElementsByTagName("body")[0].addEventListener('keyup', function () {
	        localStorage.setItem('widgeditor' + window.location.pathname, this.innerHTML);
	    });
        //this.theIframe.contentWindow.document.getElementsByTagName("body")[0].innerHTML = localStorage.getItem('widgeditor' + window.location.pathname);
    }
...
function FormReset() {
    ...
    if (typeof (Storage) !== "undefined") {
         localStorage.setItem('widgeditor' + window.location.pathname, "");
    }
    console.log("reset1");
    return true;
}

widgEditor.prototype.getInput = function () {
    var self = this;
    self.updateWidgInput();
    return self.theInput.value;
}
...
widgToolbar.prototype.addButton = function(theID, theClass, theLabel, theAction)
{
	...
	var theText = document.createElement("i");
	theText.className = theLabel;
...
{
	var theToolbar = this.parentNode.parentNode.widgToolbarObject;widgToolbarAction
...
function widgToolbarCheckState(theWidgEditor, resubmit) 
{
	...
	/* IE selections */
	if (theWidgEditor.theIframe.contentWindow.document !== null  && theWidgEditor.theIframe.contentWindow.document.selection)
	{
