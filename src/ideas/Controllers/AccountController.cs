...
using ideas.Models;
using ideas.Models.AccountViewModels;
...
namespace ideas.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Edit() {
            ...
            owner = context.Users.Include(r => r.Image).Where(r => r.Id == owner.Id).FirstOrDefault();
                ...
                var emailcheck = await _userManager.FindByEmailAsync(vm.ApplicationUser.Email);
                 ...
                 if (emailcheck == null || (emailcheck.Email == vm.ApplicationUser.Email)) {
                  var owner = await _userManager.GetUserAsync(HttpContext.User);
                    ...
                    var error = new IdentityError { Description = "E-mail address is already in use." };
        ...
        [Authorize]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult>  UploadProfileImage(IEnumerable<IFormFile> files) {
            var owner = await _userManager.GetUserAsync(HttpContext.User);
            ...
            await context.SaveChangesAsync();
            var JsonData = JsonConvert.SerializeObject(imageslist, Formatting.Indented, new JsonSerializerSettings { ContractResolver = new CamelCasePropertyNamesContractResolver() });
